...
SELECT DISTINCT CONCAT(m.surname,' ', m.firstname) AS member_name, f.name AS court_name
FROM Members AS m
...
SELECT CONCAT(m.surname,' ',m.firstname) as member_name, f.name AS facility,
...
SELECT CONCAT(m.surname,' ',m.firstname) AS member_name, bc.facility, bc.total_cost
FROM (
